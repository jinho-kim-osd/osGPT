...
def open_page_in_browser(url: str) -> WebDriver:
    ...
    if selenium_web_browser == "firefox":
        ...
        if platform == "linux" or platform == "linux2":
            ...
            chromium_driver_path = Path("/usr/lib/chromium-browser/chromedriver")
        elif platform == "darwin":
            chromium_driver_path = Path("/opt/homebrew/bin/chromedriver")
        else:
...
async def browse_website(url: str) -> Tuple(str, List[str]):
    """Browse a website"""
    driver = None
    try:
        ...
        # if "net::" in msg:
        #     raise BrowsingError(
        #         f"A networking error occurred while trying to load the page: " + re.sub(r"^unknown error: ", "", msg)
        #     )
